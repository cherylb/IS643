...
Data Source: 
https://data.world/socialmediadata/beer advocate
...
Our raw data contains `r nrow(beerdata)` data points, and a ratings matrix for this data would be 28762 X 42719. By removing beers that have received only one rating, and users that have rated less than 3 beers we can reduce the size significantly. 

```{r echo=FALSE, message=FALSE}
#remove items with 1 rating
v <- sparseratings%>% filter(count != 1)
w <- sparseusers%>% filter(count > 2)
dfbeerrm <- beerdata%>%
...
The new data set containing only users that have rated 3 or more beers, and only beers with more than 1 rating has been reduced to `r nrow(dfbeerrm) ` rows, with `r items_n ` items and `r users_n` users. By removing these items we have reduced the ratings matrix to a more manageable size of 15831 X 27479.  
...
# 2. restulst of the model against all beers in the beer profile list
# 3. top 10 rated beers as the recommendation from the profilwe
...
user2 <- as.character(users[4000,1][[1]])

model1_user1 <- model1(train_beer, user1, dfbeerprof)
model1_user2 <- model1(train_beer, user2, dfbeerprof)
```
Running model1 for the user `r user1` and `r user2` results in the following recommendations:
```{r}
...
```{r}
  ...
  target <- ifelse(dfcompare1_2$review_overall > 2.9, 1, 0) #reprsents positive rating
  predbeer <- ifelse(dfcompare1_2$predict_beers >2.9, 1,0)
  
  (cm1<-confusionMatrix(data=predbeer, 
  reference=target))
...
For the item-based collaborative filtering method I am using the recoomenderlab package, and using the ICBF algorithm, normalizing the data to remove user-rating bias, and using the cosine distance method. 
...
```{r}
target <- ifelse(df_compare2$rate > 4, 1, 0) #reprsents positive rating
...
Model1 produces the following for `r user1`
```{r}
#User1
model1_user1[[3]]
