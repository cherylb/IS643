---
title: "Project3 - evaluating recommender systems"
...
The beer data is sparse, as there are `r nrow(sparseratings %>% filter(count <200))` beers that have received less than 200 ratings. This accounts for`r nrow(sparseratings %>% filter(count <200))/nrow(sparseratings)` of the total number of items. 
...
By removing beers that have received less than 200 ratings and users that have rated less than 100 of the remaining beers we can reduce the size significantly. 
...
```{r}
#keep only items with more than than 200 ratings, and of those only users rating more than 100. 
...
The new data set containing only users that have rated more than 100beers, and only beers with more than 200 ratings has been reduced to `r nrow(dfbeerrm) ` rows, with `r items_n ` items and `r users_n` users. By removing these items we have reduced the ratings matrix to a more manageable size. 
